...
export default function Checkout() {
  ...
  function openWhatsapp() {
    const phone = "5553991148588";
    const waMessage = encodeURIComponent(
      `Olá! Gostaria de acompanhar meu pedido feito no app.
Endereço: ${address} ${complement ? "- " + complement : ""}
Agendamento: ${scheduledAt || "Não agendado"}
Forma de pagamento: ${
        paymentMethods.find((p) => p.id === paymentMethodId)?.methodName || ""
      }`
    );
    ...
    <>
      <main className="max-w-lg mx-auto py-8 px-4">
        ...
        <div className="bg-white rounded-lg shadow p-4 mb-6">
          ...
          {user && (
            <div className="mb-3 p-2 bg-blue-50 border border-blue-200 rounded-md">
              <p className="text-xs text-blue-700">
                Os campos abaixo vêm do seu cadastro, mas você pode editá-los para este pedido específico.
              </p>
            </div>
          )}
          <input
          ...
          {showScheduler && (
            <div className="mt-3 animate-in fade-in duration-300">
              <label className="block text-xs text-gray-600">Selecione a data e hora:</label>
              <input
                type="datetime-local"
        ...
        <div className="bg-white rounded-lg shadow p-4 mb-6">
          ...
          <p className="text-xs text-gray-500 mt-2">O pagamento é realizado no momento da entrega.</p>
        </div>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60 backdrop-blur-sm">
          <div className="bg-white rounded-xl shadow-lg p-6 md:p-8 text-center max-w-sm w-full mx-4 animate-in fade-in-90 zoom-in-95">
            <h2 className="text-2xl font-bold mb-3 text-green-600">Pedido Realizado!</h2>
            <p className="mb-6 text-gray-700">
              Seu pedido foi enviado com sucesso. Para confirmar e acompanhar a entrega, fale conosco no WhatsApp.
            </p>
            <button
